...
export default class DataSource extends Array {
    constructor() {
        ...
        this.forEach(source => {
            // La vérification du cache a été déplacée à l'intérieur de fetchData 
            // pour gérer proprement l'asynchronisme d'IndexedDB
            source.fetch = async () => { return await fetchData(source) };
        });
    } 
...
async function fetchData(source) {
    ...
    const response = await fetch('./assets/' + source.ctFile);
    if (!response.ok)
        return data;
    ...
    if (source.usagesFile) {
        const usagesResponse = await fetch('./assets/' + source.usagesFile);
